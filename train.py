...
### pretrained
# pretrained_dict = torch.load('./pkl/net_epoch_43-Flow-Network.pkl')
# model_dict = Flownet.state_dict()
# pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
# model_dict.update(pretrained_dict)
# Flownet.load_state_dict(model_dict)
#
# pretrained_dict = torch.load('./pkl/net_epoch_43-Seg-Network.pkl')
...
opt_flow2 = torch.optim.Adam(Flownet.parameters(),lr=0.0001)
...
for epoch in range(200):
    ...
    for step, (img_ed,img_pre,img_mid,img_aft,img_es,labeled,labeles) in enumerate(dataloder):
        ...
        #reg1
        warped_ed, flow_field, speed_field, step1_flow, step2_flow, step3_flow, step4_flow,endo = Flownet(img_es,img_ed)
############
...
################################
        ...
        opt_seg.zero_grad()
        fuse_es_seg, fuse_ed_seg, es_seg, ed_seg, es_seg_flow, ed_seg_flow = Segnet(img_es, img_ed, flow_field)
        ...
        img_seg_es=es_seg[0, 1, :, :, :] * 1 + es_seg[0, 2, :, :, :] * 2 + es_seg[0, 3, :, :, :] * 3
        img_seg_ed=ed_seg[0, 1, :, :, :] * 1 + ed_seg[0, 2, :, :, :] * 2 + ed_seg[0, 3, :, :, :] * 3

        img_seg_es=img_seg_es.data.cpu().numpy()
        img_seg_ed = img_seg_ed.data.cpu().numpy()

        img_seg_es_1 = sitk.GetImageFromArray(img_seg_es)
        # img_seg_ed_1 = sitk.GetImageFromArray(img_seg_ed)
        spacing = np.array(img_seg_es_1.GetSpacing())[[2, 1, 0]]

        spacing_target = (16, 1.25, 1.25)
        spacing_target = list(spacing_target)
        # spacing_target[0] = spacing[0]

        img_seg_ed = data_utils.read_image(img_seg_ed, spacing, spacing_target)
        img_seg_es = data_utils.read_image(img_seg_es, spacing, spacing_target)
        img_seg_ed = torch.from_numpy(img_seg_ed)
        img_seg_es = torch.from_numpy(img_seg_es)
        # img_seg_es= data.convert_to_one_hot(img_seg_es)
        # img_seg_ed = data.convert_to_one_hot(img_seg_ed)
        #img_seg_es = img_seg_es.to(device).float()
        #img_seg_ed = img_seg_ed.to(device).float()

        loss_seg = loss_seg_ce + loss_seg_dice+loss_motion
        # loss_seg.backward()
        # opt_seg.step()
        ...
        #reg2
        warped_ed_2, flow_field_2, speed_field_2, step1_flow_2, step2_flow_2, step3_flow_2, step4_flow_2, endo_2 = Flownet(img_seg_es,img_seg_ed)
        ############
        loss_smooth = 0.5 * (criterion_grad(flow_field_2[:, :, :, mi3:mx3, mi4:mx4]) + \
                             criterion_grad(flow_field_2[:, :, :, :, :]))
        ##############
        loss_reg_ncc = ((criterion_ncc(img_seg_ed[:, :, 1:-1, mi3:mx3, mi4:mx4], warped_ed_2[:, :, 1:-1, mi3:mx3, mi4:mx4]) + \
                         criterion_ncc(img_seg_es[:, :, 1:-1, mi3:mx3, mi4:mx4], endo_2[:, :, 1:-1, mi3:mx3, mi4:mx4])) / 2)
                        # (criterion_ncc(img_aft[:, :, 1:-1, mi3:mx3, mi4:mx4],
                        #                step1_flow[:, :, 1:-1, mi3:mx3, mi4:mx4]) + \
                        #  criterion_ncc(img_mid[:, :, 1:-1, mi3:mx3, mi4:mx4],
                        #                step2_flow[:, :, 1:-1, mi3:mx3, mi4:mx4]) + \
                        #  criterion_ncc(img_pre[:, :, 1:-1, mi3:mx3, mi4:mx4],
                        #                step3_flow[:, :, 1:-1, mi3:mx3, mi4:mx4]) + \
                        #  criterion_ncc(img_seg_ed[:, :, 1:-1, mi3:mx3, mi4:mx4],
                        #                step4_flow[:, :, 1:-1, mi3:mx3, mi4:mx4])) / 4) / 2
        ##############
        # speed_combin = 0.25 * (speed_field[:, 0:3, :, :, :] + speed_field[:, 3:6, :, :, :] + \
        #                        speed_field[:, 6:9, :, :, :] + speed_field[:, 9:12, :, :, :])
        # fm = flow_field - flow_field.mean()
        ...
        ###############
        loss_reg2 = loss_smooth + loss_reg_ncc
        loss_motion=loss_reg_ncc


        loss_reg.backward(retain_graph=True)
        loss_seg.backward()
        loss_reg2.backward(retain_graph=True)
        opt_flow.step()
        opt_seg.step()
        opt_flow2.step()
        ...
        if step % 1 == 0:
            ...
            with torch.no_grad():
                  print_state.printst(step, img_ed, img_es, img_pre, img_mid, img_aft, labeled, labeles, \
                                     ...
                                     fuse_es_seg, fuse_ed_seg, es_seg, ed_seg, es_seg_flow, ed_seg_flow)
